...
# ==============================================================================
# DEPLOYMENT SCRIPT: Intuitive Server Deployment
...
DC_DOWN_OUTPUT=$(docker compose -f docker-compose.yml -f docker-compose.dev.yml down 2>&1)
...
DC_BUILD_OUTPUT=$(docker compose -f docker-compose.yml -f docker-compose.dev.yml build --no-cache 2>&1)
...
# Capture output quietly
DC_UP_OUTPUT=$(docker compose -f docker-compose.yml -f docker-compose.dev.yml up -d --remove-orphans 2>&1)
...
STARTUP_WAIT_OUTPUT=$(timeout 60 docker compose -f docker-compose.yml -f docker-compose.dev.yml logs -f 2>&1 | grep -m 1 "Application startup complete.")
